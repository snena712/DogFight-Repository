...
void SkyBox::Init(const char* _filename, const char* _vsfile, const char* _psfile, const char* _texfolder) {
...
void SkyBox::Draw()
{
	...
	devcontext->RSSetState(RSCullNone);
	
	// 描画
	ModelData* md = ModelResourceManager::GetInstance()->GetResource(m_assimpfileName);
	if (md != nullptr) {
		md->Draw(devcontext, make, _tex);
	}
}

bool SkyBox::InitTex2D(const char* _filename,
	const char* _vsfile,
	const char* _psfile,
